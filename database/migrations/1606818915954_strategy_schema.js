...
class StrategySchema extends Schema {
  up () {
    this.create('strategies', (table) => {
      ...
      if(process.env.NODE_ENV === 'test') table.integer('exchange_id').unsigned().references('id').inTable('exchanges').onDelete('CASCADE')
      else table.integer('exchange_id').unsigned().references('id').inTable('exchanges')
